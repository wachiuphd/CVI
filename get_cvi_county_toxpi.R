...
############ County - take median census tract
...
lat.df <- aggregate(. ~ STATE+County_Name+GEOID.State+GEOID.County,
                           data=as.data.frame(cvi.df)[,c(1,2,3,4,ncol(cvi.df)-1)],
                           FUN = median, na.rm=T)
long.df <- aggregate(. ~ STATE+County_Name+GEOID.State+GEOID.County,
                    data=as.data.frame(cvi.df)[,c(1,2,3,4,ncol(cvi.df))],
                    FUN = median, na.rm=T)
latlong.df <- left_join(lat.df,long.df)
latlong.df$LatLong <- paste0("+",latlong.df$Lat,",",latlong.df$Long)
cvi.county.df$LatLong <- latlong.df$LatLong
...
fwrite(cvi.pct.toxpi,file.path(pctdir,"CVI-county-pct-allinone.csv"))
...
fwrite(cvi.pct.toxpi.gis,file.path(pctdir,"CVI-county-pct-allinone.gis.csv"))

pdf(file.path(pctdir,"ToxPi-county-pct-allinone.pdf"),height=8,width=10)
...
pdf(file.path(pctdir,"ToxPi-county-pct-subcat.pdf"),height=8,width=10)
for (i in 1:length(categories)) {
  onecat <- categories[i]
...
pdf(file.path(pctdir,"ToxPi-county-pct-subcat-comb.pdf"),height=8,width=10)
...
fwrite(cvi.pct.toxpi.comb,file.path(pctdir,paste0("CVI-county-pct-comb.csv")))
# save for use by ToxPi GIS
...
fwrite(cvi.pct.toxpi.comb.gis,file.path(pctdir,paste0("CVI-county-pct-comb.gis.csv")))
...
names(cvi.pct.df.namesfixed)<-gsub("\"","",names(cvi.pct.df.namesfixed))
fwrite(cvi.pct.df.namesfixed,file.path(pctdir,"CVI-county_data_pct.gis.csv"))

...
for (i in 1:length(categories)) {
  onecat <- categories[i]
  if (substr(onecat,1,2)=="CC") {
    categoryname <- paste(str_replace(onecat,"CC","Climate Change Risk"))
  } else {
    categoryname <- paste("Baseline Vulnerability:",onecat)
  }
  print(categoryname)
  
  subcategories <- unique(indicators.df$Subcategory[
    indicators.df$`Category`==onecat])
  numsubcat <-length(subcategories)
  cvi.pct.toxpi.cat  <- fread(file.path(pctdir,
                              paste0("CVI-pct-cat-",
                                     gsub(": ","-",onecat),".csv")),
                              keepLeadingZeros = TRUE,integer64 = "numeric")
  indx<-c(3,5+0:numsubcat)
  scores.df <- cvi.pct.toxpi.cat[,..indx]
  names(scores.df)[1] <- "GEOID.Tract"
  names(scores.df)[2] <- "Category Score"
  ...
  for (j in 1:(numsubcat+1)) {
    ...
    if (j == 1) {
      plt$title<-paste0("     ",categoryname)
    } else {
      plt$title<-paste0("     ",names(scores.df.med.county)[j+1])
    }
    plt$ggplot_polygon <- geom_polygon(aes(fill = value,color=value))
    if (j == 1) catmap.list[[1]] <- plt$render() else subcatmap.list[[j-1]] <- plt$render()
  }

  indx <- 1+1:(numsubcat)
  numrow <- ceiling((numsubcat)/2)
  figheight <- 1.75*(numrow+2)
  figmap <- ggarrange(catmap.list[[1]],
                      ggarrange(plotlist=subcatmap.list,
                                nrow=numrow,ncol=2,labels=letters[indx]),
                      labels=c(letters[1],""),
                      nrow=2,ncol=1,heights = c(2,numrow))
  ggsave(file.path(figdir,
                   paste0("CVI_category-",i,"-",
                          gsub(": ","-",as.character(onecat)),"-map.pdf")),
  ...
  scoresboxplt<-
    ...
    ggtitle(categoryname)
  print(scoresboxplt)
  
  ##### Geographic scale
  cat.indicators <- subset(indicators.df,`Category` == onecat)
  cat.indicators$Subcategory <- factor(cat.indicators$Subcategory,
                                       levels=unique(cat.indicators$Subcategory))
  indicators.geo <- 
    rbind(
      as.matrix(t(table(cat.indicators$GeographicScale))),
      as.matrix(ftable(cat.indicators$Subcategory,
                       cat.indicators$GeographicScale)))
  rownames(indicators.geo)[1] <- "Category Score"
  ...
  r2.df <- rbind(data.frame(Variable=rep("State",1+numsubcat),
                            Subcategory=names(state.r2),
                            R2=state.r2),
                 data.frame(Variable=rep("County",1+numsubcat),
                            Subcategory=names(county.r2),
                            R2=county.r2)
  )
  
  
  r2.df$Subcategory<-factor(r2.df$Subcategory,levels=
                           rev(names(state.r2)))
  pr2<-ggplot(r2.df)+
    geom_col(aes(x=Subcategory,y=R2,
  ...
  ggsave(file.path(figdir,
                   paste0("CVI_category-",i,"-",
                          gsub(": ","-",as.character(onecat)),"-summary.pdf")),
